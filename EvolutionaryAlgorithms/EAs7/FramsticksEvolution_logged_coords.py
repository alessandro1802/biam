...
def frams_evaluate(frams_lib, individual):
	...
	def calculate_distance_torch(point1, point2):
		return torch.norm(point1 - point2)

	def calculate_angle_torch(v1, v2):
		cos_theta = torch.dot(v1, v2) / (torch.norm(v1) * torch.norm(v2))
		angle = torch.acos(cos_theta)
		return angle

	def evaluate_cube_similarity_torch(shape):
		target_n = 8
		distance_w = 0.8
		angle_w = 0.2

		points = torch.tensor(shape[1:], dtype=torch.float32)  # Excluding the center of mass
		n = len(points)
		...
		for i, j in combinations(range(n), 2):
			distances.append(calculate_distance_torch(points[i], points[j]))
		distances = torch.tensor(distances)
		# Get unique distances and count their occurrences
		unique_distances, counts = torch.unique(distances.round(decimals=4), return_counts=True)
		# Determine edge length uniformity score
		if len(unique_distances) < 1:
			edge_length_uniformity = 0.0
		else:
			# The primary edge length should be the most frequent unique distance
			primary_edge_length = unique_distances[0]
			# Calculate the variance of distances from the primary edge length
			edge_length_deviation = torch.var(distances - primary_edge_length)
			edge_length_uniformity = 1 / (1 + edge_length_deviation)
		# Check for right angles
		angles = []
		for p1, p2, p3 in combinations(points, 3):
			v1 = p2 - p1
			v2 = p3 - p1
			angle = calculate_angle_torch(v1, v2)
			angles.append(angle)
		angles = torch.tensor(angles)
		# Right angles should be approximately pi/2 radians (90 degrees)
		right_angle = torch.tensor(torch.pi / 2)
		angle_deviation = torch.mean((angles - right_angle).abs())
		# The similarity score for angles
		angle_correctness = 1 / (1 + angle_deviation)
		# Combine the scores (weighted average)
		# similarity_score = (edge_length_uniformity + angle_correctness) / 2
		similarity_score = edge_length_uniformity * distance_w + angle_correctness * angle_w
		return similarity_score.item() + 1

	valid = True
	try:
		# points = data[0]["evaluations"]['']["data->bodyrecording"]
		...
		shape = default_evaluation_data["data->bodyrecording"][0]
		fitness = [evaluate_cube_similarity_torch(shape)]
